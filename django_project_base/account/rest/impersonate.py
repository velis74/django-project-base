...
class ImpersonateUserDialogSerializer(serializers.Serializer):
    template_context = dict(url_reverse="profile-base-impersonate-user", dialog_header_classes="bg-info")
    form_titles = {
        "table": "",
        "new": "Search for user",
        "edit": "",
...
class ImpersonateUserViewset(viewsets.SingleRecordViewSet):
    ...
    @action(
        detail=False, methods=["post"], url_name="impersonate-start", permission_classes=[IsAuthenticated, IsAdminUser]
    )
