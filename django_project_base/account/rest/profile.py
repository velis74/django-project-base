...
from django.db.models import ForeignKey, Model, QuerySet, Value
...
class ProfileSerializer(DynamicModelSerializerMixin, ModelSerializer):
    ...
    last_changelog = fields.AutoGeneratedField(display=DisplayMode.HIDDEN)
...
class ProfileRegisterSerializer(ProfileSerializer):
    email = AutoGeneratedField(required=True, allow_blank=False)
    password = fields.CharField(label=_("Password"), password_field=True)
    password_repeat = fields.CharField(label=_("Repeat Password"), password_field=True, write_only=True)
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            project = swapper.load_model("django_project_base", "Project").objects.get(
                slug=getattr(
                    self.request, settings.DJANGO_PROJECT_BASE_BASE_REQUEST_URL_VARIABLES["project"]["value_name"], None
                )
            )
        except:
            project = None
