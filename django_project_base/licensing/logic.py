...
class LogAccessService:
    def log(self, user_profile, channels: List[str], object_pk: str, on_sucess=None):
        ...
        current_items = LicenseAccessUse.objects.filter(
            user_id=str(user_profile.pk), content_type=content_type
        )
        _channels = list(
            content_type_model.objects.annotate(
                pkstr=Cast(
                    content_type_model._meta.pk.name, output_field=models.CharField()
                )
            )
            .filter(
                pkstr__in=current_items.values_list("content_type_object_id", flat=True)
            )
        ...
        chl_prices = {
            i.name: i.notification_price for i in ChannelIdentifier.supported_channels()
        }
        used = 0
        for used_channel in list(set(list(ch_one.keys()) + list(ch_two.keys()))):
            used += chl_prices.get(used_channel, 0) * (
                ch_one.get(used_channel, 0) + ch_two.get(used_channel, 0)
            )
