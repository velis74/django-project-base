            ...
            try:
                ...
                if view:
                    ...
                    txt: str = format_permission_classes(
                        getattr(_view, 'initkwargs', {}).get('permission_classes', [])) or format_permission_classes(
                        getattr(getattr(_view, 'cls', object()), 'permission_classes', []))
                    if not txt:
                        continue
...
django_project_base_router.register(r'%s' % REST_API_CONFIG.ProjectRole.url, ProjectRoleViewSet,
                                    basename=REST_API_CONFIG.ProjectRole.basename)
